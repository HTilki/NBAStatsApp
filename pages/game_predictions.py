...
if metadata.get("game_count", 0) == 0:
    st.warning("No predictions available at this time.")
else:
    if metadata:
    ...
    with st.expander("View Upcoming Game Predictions"):
        ...
        for game in games:
            ...
            winning_team_abbreviation = get_abbreviation(
                ...
                team_name=game["prediction"]["winner_name"],
            )
        ...
        games_df = (
            ...
            .sort("date")
            .unique(
                subset=["home_team", "away_team"], keep="first", maintain_order=True
            )
        )
